...
def get_text(
    ...
    connection = http.client.HTTPSConnection(target.hostname, target.port, timeout=connect_timeout)
    try:
        connection.connect()
        if connection.sock is not None:
            connection.sock.settimeout(request_timeout)
        connection.request("GET", target.path or "/", headers={"User-Agent": "QuoteMux/eastmoney_official"})
        response = connection.getresponse()
        if response.status == 429:
            raise P0ProviderError("rate_limit_error", "Eastmoney 返回 429")
        if response.status in {401, 403}:
            raise P0ProviderError("permission_error", f"Eastmoney 返回 {response.status}")
        if response.status < 200 or response.status >= 300:
            raise P0ProviderError("request_error", f"Eastmoney 返回 HTTP {response.status}")
        body = response.read(max_bytes + 1)
        if len(body) > max_bytes:
            raise P0ProviderError("contract_error", "Eastmoney 响应超过 max response bytes")
